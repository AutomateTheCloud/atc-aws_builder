#!/bin/bash
###------------------------------------------------------------------------------------------------
# alias:   aws_builder.rds.postgresql.create
# script:  create.sh
# purpose: Creates PostgreSQL Database (supports snapshots and SQL execution)
# version: 1.0.0
###------------------------------------------------------------------------------------------------

###------------------------------------------------------------------------------------------------
## Config
declare -r SELF_IDENTITY="postgresql_create"
declare -r SELF_IDENTITY_H="RDS (PostgreSQL - Create)"
...
REQUIRED_SOURCE_FILES+=(
   ...
   "${LIB_FUNCTIONS_AWS_RDS_POSTGRESQL}"
)
...
load_info_postgresql "${TMP_FILE_CLOUDFORMATION_OUTPUT}" "${AWS_REGION}"
RETURNVAL="$?"; if [ ${RETURNVAL} -ne 0 ]; then exit_logic ${RETURNVAL}; fi

log_notice "Checking to see if another PostgreSQL Database occurrence exists"
rds_get_current_database_id DB_PREVIOUS_OCCURRENCE "${POSTGRESQL_POSTGRESQLREFERENCENAME}" "${AWS_REGION}"
RETURNVAL="$?"; if [ ${RETURNVAL} -ne 0 ]; then exit_logic ${RETURNVAL}; fi

if(! is_empty "${DB_PREVIOUS_OCCURRENCE}"); then
    log_notice "- Previous PostgreSQL Database Occurrence detected [${DB_PREVIOUS_OCCURRENCE}], we will attempt to replace after new PostgreSQL Database is created"
    line_break
fi
## END Initialize
###------------------------------------------------------------------------------------------------
...
if(! is_empty "${SNAPSHOT_ARN}"); then
    log_notice "Creating PostgreSQL Database from Snapshot"
    postgresql_create_from_snapshot "${SNAPSHOT_ARN}" "${AWS_OPERATION_TIMEOUT}" "${AWS_REGION}"
    RETURNVAL="$?"
    if [ ${RETURNVAL} -ne 0 ]; then
    ...
    log_notice "Creating PostgreSQL Database"
    postgresql_create "${AWS_OPERATION_TIMEOUT}" "${AWS_REGION}"
    RETURNVAL="$?"
    if [ ${RETURNVAL} -ne 0 ]; then
        TMP_ERROR_MSG="Failed to create PostgreSQL Database, cannot continue"
        log_error "- ${TMP_ERROR_MSG}"
        log_warning "Performing Cleanup"
        rds_delete "${POSTGRESQL_POSTGRESQLDBID}" no "${AWS_REGION}"
        exit_logic $RETURNVAL "${TMP_ERROR_MSG}"
...
rds_get_endpoint DB_ENDPOINT "${POSTGRESQL_POSTGRESQLDBID}" "${AWS_REGION}"
RETURNVAL="$?"
if [ ${RETURNVAL} -ne 0 ]; then
...
postgresql_verify_connection "${DB_ENDPOINT}" "${POSTGRESQL_MASTERUSERNAME}" "${POSTGRESQL_MASTERPASSWORD}" "${POSTGRESQL_DBNAME}" "${POSTGRESQL_POSTGRESQLPORT}"
RETURNVAL="$?"
if [ ${RETURNVAL} -ne 0 ]; then
...
if(! is_empty "${DIRECTORY_SQL_EXECUTION}"); then
    ...
    for TMP_FILE_SQL in $(find "${DIRECTORY_SQL_EXECUTION}" -type f -name "*.sql" | grep ".sql\$" | sort); do
        ...
        postgresql_execute_sql "${DB_ENDPOINT}" "${POSTGRESQL_MASTERUSERNAME}" "${POSTGRESQL_MASTERPASSWORD}" "${POSTGRESQL_DBNAME}"  "${POSTGRESQL_POSTGRESQLPORT}" "${TMP_FILE_SQL}"
        RETURNVAL="$?"
        if [ ${RETURNVAL} -ne 0 ]; then
...
for (( REPLICA_ITERATION=1; REPLICA_ITERATION<=${POSTGRESQL_REPLICACOUNT}; REPLICA_ITERATION++ )); do
    log_notice "Creating PostgreSQL Database [Iteration: ${REPLICA_ITERATION}]"
    postgresql_create_replica "${REPLICA_ITERATION}" "${AWS_OPERATION_TIMEOUT}" "${AWS_REGION}"
    RETURNVAL="$?"
    if [ ${RETURNVAL} -ne 0 ]; then
        TMP_ERROR_MSG="Failed to create PostgreSQL Replica Database [Iteration: ${REPLICA_ITERATION}], cannot continue"
        log_error "- ${TMP_ERROR_MSG}"
        log_warning "Performing Cleanup"
        rds_delete "${POSTGRESQL_POSTGRESQLDBID}" no "${AWS_REGION}"
        exit_logic $RETURNVAL "${TMP_ERROR_MSG}"
...
if(! is_empty "${DB_PREVIOUS_OCCURRENCE}"); then
    log_notice "- Previous PostgreSQL Database Occurrence detected [${DB_PREVIOUS_OCCURRENCE}], performing cleanup"
    call_sleep 60 "giving application time to switch over to the new DNS endpoint"
    if [[ "ZZ_$(to_upper "${POSTGRESQL_ENVIRONMENT}")" == "ZZ_PRD" ]]; then
        TAKE_SNAPSHOT=yes
    fi
    rds_delete "${DB_PREVIOUS_OCCURRENCE}" "${TAKE_SNAPSHOT}" "${AWS_REGION}"
    RETURNVAL="$?"
    if [ ${RETURNVAL} -ne 0 ]; then
        TMP_ERROR_MSG="Failed to delete previous PostgreSQL Database Occurrence, manual intervention required"
        log_error "- ${TMP_ERROR_MSG}"
        exit_logic $RETURNVAL "${TMP_ERROR_MSG}"
    fi
fi
